"""
dynamic_sankey.generator
------------------------
Core HTML generator for D3.js-based animated Sankey diagrams.

Generates a fully self-contained, CDN-linked standalone HTML file.
No server required. No Plotly. Pure D3 SVG morphing.
"""
...
def validate_config(config: dict) -> None:
    """
    Validate a sankey config dict. Raises ValueError on any inconsistency.

    Checks:
    - nodes and links are non-empty
    - every link's source/target ID exists in nodes
    - every time_series entry has exactly len(links) values
    """
    ...
    if not nodes:
        raise ValueError("config['nodes'] must contain at least one node.")
    if not links:
        raise ValueError("config['links'] must contain at least one link.")
    if not time_series:
        raise ValueError("config['time_series'] must contain at least one entry.")
    ...
    for i, link in enumerate(links):
        if link["source"] not in node_ids:
            raise ValueError(
                f"links[{i}]: source ID {link['source']!r} not found in nodes."
            )
        if link["target"] not in node_ids:
            raise ValueError(
                f"links[{i}]: target ID {link['target']!r} not found in nodes."
            )

    for ts in time_series:
        if len(ts["values"]) != n_links:
            raise ValueError(
                f"time_series '{ts['label']}': "
                f"got {len(ts['values'])} values, expected {n_links} (= len(links))."
            )


def generate_html(config: dict, title: str = "Dynamic Sankey") -> str:
    """
    Generate a standalone HTML file with an animated D3.js Sankey diagram.

    Args:
        config: Dict with keys:
            - nodes:       list of {id, label, color}
            - links:       list of {source, target}  (topology, fixed across time)
            - time_series: list of {label, values}   (values align with links list)
        title:  Title shown above the diagram (default: "Dynamic Sankey")

    Returns:
        HTML string — write to any .html file, open in browser, done.

    Example:
        >>> from dynamic_sankey import generate_html
        >>> from my_config import SANKEY_CONFIG
        >>> html = generate_html(SANKEY_CONFIG, title="My Flows 2024")
...
# ---------------------------------------------------------------------------
# HTML / D3 template  (r-string avoids Python escape conflicts with JS code)
# ---------------------------------------------------------------------------
